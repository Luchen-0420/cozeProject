...
**在开始本课程前，请先阅读本课程的[task文档](./task.md)！！！**
...
1. 首先我们要进行一个查询"未阅读"飞书表格记录

2. 然后统计+输出（这部分随着表格记录的增加，大模型节点可能承受不住，选择代码节点来实现，或者循环节点？）

3. 至于如何定时推送，bot是有定时任务，还得摸一下。

4. 另外关于推荐阅读准确度问题，好像找到解决方法，大概因为我给定的参考参数太多（title,summary,tag,modality）。
   换个思路，在普通场景下实现快速搜索的时候是按照关键词搜素。
   我们已经给定了tag，也就是我们的关键词。实际上只按照tag去匹配，精准度应该高点。

5. 还有关于节点内如何使用循环节点来达到官方自带的批处理功能需要琢磨下。
